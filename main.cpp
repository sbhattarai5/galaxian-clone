...
int game_execute(Surface & surface, Event & event)
{
    Image * spaceship_img = new Image("images/galaxian/GalaxianGalaxip.gif");
    //Image * aquaalien_img = new Image("images/galaxian/GalaxianAquaAlien.gif");
    //Image * purplealien_img = new Image("images/galaxian/GalaxianPurpleAlien.gif");
    //Image * redalien_img = new Image("images/galaxian/GalaxianRedAlien.gif");
    Star stars[N_stars];
    int RATE = 500/60;
    int level = 1;
    SpaceShip spaceship;
    //AquaAlien test_aqua_alien(W/2, 40, 1, 0);
    //PurpleAlien test_purple_alien(W/2, 20, 1, 0);
    //RedAlien test_red_alien(W/2, 0, 1, 0);
    std::list< Laser > laser_list;
    Fleet * fleet = new Fleet(level);
    int next_fleet_delay = 500;
    int last_fleet_elapsed_time = 0;
    std::list< Explosion > explosions;
    bool spaceship_reset = false;
    int reset_time = 0;
    int reset_i = 1;
    while (1)
    {
        ...
        KeyPressed keypressed = get_keypressed();
        if (event.poll() && event.type() == QUIT) break;
        if (spaceship.lives() == 0) return spaceship.score();
        if (keypressed[LEFTARROW]) spaceship.move_left();
        else if (keypressed[RIGHTARROW]) spaceship.move_right();
        if (keypressed[SPACE] && fleet->no_of_aliens_alive() != 0 && !spaceship_reset) spaceship.shoot(laser_list);
        surface.lock();
        surface.fill(BLACK);
        print_stars(stars, surface);
        print_lasers(laser_list, surface);
        if (!spaceship_reset) spaceship.print(surface);
        else
        {
            ++reset_time;
            if (reset_time > 150)
            {
                spaceship_reset = false;
                reset_time = 0;
                reset_i = 0;
            }
            if (reset_time <= (reset_i * 10))
            {
                spaceship.print(surface);
            }
            else
            {
                if (reset_time % 10 == 0) reset_i += 2;
            }
        }
        draw_health(fleet, surface);
        draw_lives(spaceship.lives(), spaceship_img, surface);
        draw_explosion(explosions, surface);
        fleet->print(surface);
        print_score(spaceship, surface);
        if (fleet->no_of_aliens_alive() == 0 && laser_list.size() == 0)
        {
            ++last_fleet_elapsed_time;
            if (last_fleet_elapsed_time >= next_fleet_delay)
            {
                ++level;
                if (level > 4) fleet = new Fleet(4);
                else fleet = new Fleet(level);
                last_fleet_elapsed_time = 0;
            }
            else
            {
                print_new_level(level, surface);
            }
        }
        surface.unlock();
        surface.flip();
        make_stars_fall(stars);
        move_lasers(laser_list);
        fleet->move(laser_list);
        check_collison(fleet, laser_list, spaceship, explosions, spaceship_reset);
        spaceship.increase_last_bullet_fired_time();
...
void  game_loop()
{
    ...
    while (1)
    {
        if (game_state == 0)
        {
            score = game_execute(surface, event);
            ++game_state;
        }
        if (game_state == 1)
        {
            //user_choice = end_screen(surface, score, event);
            //if (user_choice) game_state = 0;
            //else game_state = 2;
